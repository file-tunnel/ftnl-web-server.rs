#![forbid(unsafe_code)]
//! The four avenues this web server can use to reach `ftnl-api-server`.
//!
...
//! to file-tunnel — the environment prefix, the service slug that names its NATS
//! subjects, and the api route that accepts an envelope.
//!
//! | # | Mode | What happens |
//! |---|------|--------------|
//! | 1 | `direct_read` | this server queries Postgres itself through `ftnl-lib-core`, read-only |
//! | 2 | `http` | stateless request to the `ftnl-api-server` cluster |
//! | 3 | `tcp` | framed, held-open connection to the `ftnl-api-server` cluster |
...
//! `ftnl-lib-core`'s read context, and hand it to [`gateway_from_env`]. See
...
///
/// `FTNL_API_URL`, `FTNL_NATS_URL`, and the rest; see the
/// table in `ores-transport`'s README.
pub const ENV_PREFIX: &str = "FTNL";
...
pub const SERVICE_SLUG: &str = "ftnl";
