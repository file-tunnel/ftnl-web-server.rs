//! Four-avenue web ↔ API binding for `file-tunnel`.
//!
//! 1. Direct read-only DB via `*-lib-core` named queries (no migrations).
//! 2. Stateless HTTP from `app.file-tunnel.dev` to `api.file-tunnel.dev`.
//! 3. Stateful TLS 1.3/mTLS TCP to `api.file-tunnel.dev:7443`.
...
pub const GITHUB_ORG: &str = "file-tunnel";
pub const ORG_SLUG: &str = "ftnl";
pub const DNS_ZONE: &str = "file-tunnel.dev";
